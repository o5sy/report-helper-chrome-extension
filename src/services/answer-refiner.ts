...
2. 어미 변경: 모든 '했다', '했습니다' 등의 서술어를 '~함.' 형태로 변경해 주세요. (예: "발표를 했다" → "발표를 했음.")
3. 꼬리 질문 처리:
    * '- '로 시작하는 문장은 멘토의 꼬리 질문입니다.
    * **(중요) 꼬리 질문이 있는 경우에만 지침 적용**
    * 학생의 답변 내용과 명확히 구분될 수 있도록 **한 줄 공백(줄바꿈)**을 추가한 후에 배치해 주세요.
    * 꼬리 질문의 내용은 학생에게 던지는 질문이므로 **존댓말**로 수정해 주세요. (예: "설명해 주세요.", "알고 있으신가요?")
    * 꼬리 질문의 시작 부분인 '- '는 그대로 유지하여 '- 질문 내용' 형태로 만들어 주세요.
    * **(중요) 꼬리 질문이 없다면 질문을 추가하거나 만들지 마세요.**
